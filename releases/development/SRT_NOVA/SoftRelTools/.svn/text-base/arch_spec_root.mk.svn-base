...
ROOTLIBS  = $(shell root-config --libs) -lEG -lTreePlayer -lGeom -lFFTW -lEGPythia6
...
cleanroot:
	rm -f $(shlibdir)lib$(PACKAGE)$(cintdir)Cint.rootmap \
	$(shlibdir)lib$(PACKAGE)$(cintdir)Cint_rdict.pcm

$(cintworkdir)$(CINTOUTPUT): $(CINTLIST) LinkDef.h
	@echo "Generating $(@F) dictionary..."
	@$(check_dep_dir)
	$(TRACE)rootcint -f $@ -s	 $(shlibdir)lib$(PACKAGE)$(cintdir)Cint.so \
    -rml lib$(PACKAGE)$(cintdir)Cint.so -rmf $(shlibdir)lib$(PACKAGE)$(cintdir)Cint.rootmap \
    -noIncludePaths \
