...
# initialization
unitTestResults = $(addprefix $(workdir)/, $(addsuffix .result, $(basename $(notdir $(UNITTESTS)))))

# rule to make (fake) results file from each unit test
$(unitTestResults) : $(workdir)/%.result : %.java
	@echo "<**running unit test**> $(basename $(notdir $<))"
	java $(basename $(notdir $<))
